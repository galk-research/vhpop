...
file_pairs=(
    ...
    "gripper-domain.pddl gripper-3.pddl"
...
for pair in "${file_pairs[@]}"; do
    ...
    cmd1="./vhpop -g $h $f -v1 examples/$domain examples/$problem"
    cmd2="./vhpop -g $h $f -y -v1 examples/$domain examples/$problem"
    cmd3="./vhpop -g $h $f -z -v1 examples/$domain examples/$problem"
    cmd4="./vhpop -g $h $f -y -z -v1 examples/$domain examples/$problem"
    


    timeout 5s $cmd1 > "$tmp1" 2>&1
    status1=$?
    timeout 5s $cmd2 > "$tmp2" 2>&1
    status2=$?
    timeout 5s $cmd3 > "$tmp3" 2>&1
    status3=$?
    timeout 5s $cmd4 > "$tmp4" 2>&1
    status4=$?
    ...
    plans1=$(extract_plans_generated "$tmp1")
    plans2=$(extract_plans_generated "$tmp2")
    plans3=$(extract_plans_generated "$tmp3")
    plans4=$(extract_plans_generated "$tmp4")

    visited1=$(extract_plans_visited "$tmp1")
    visited2=$(extract_plans_visited "$tmp2")
    visited3=$(extract_plans_visited "$tmp3")
    visited4=$(extract_plans_visited "$tmp4")

    dead1=$(extract_dead_ends "$tmp1")
    dead2=$(extract_dead_ends "$tmp2")
    dead3=$(extract_dead_ends "$tmp3")
    dead4=$(extract_dead_ends "$tmp4")

    length1=$(extract_plan_length "$tmp1")
    length2=$(extract_plan_length "$tmp2")
    length3=$(extract_plan_length "$tmp3")
    length4=$(extract_plan_length "$tmp4")
...
print_table() {
    ...
    echo ""
    echo "                      $title                         "
    echo "|       Problem       |  Default  |    pre    |  effects  |   both    |"
    echo "|---------------------|-----------|-----------|-----------|-----------|"
    for i in "${!problems[@]}"; do
        printf "| %-19s | %-9s | %-9s | %-9s | %-9s |\n" \
            "${problems[$i]}" "${arr1[$i]}" "${arr2[$i]}" "${arr3[$i]}" "${arr4[$i]}"
    done
}

# Print the four tables
print_table "Plans Generated" plans_generated_1[@] plans_generated_2[@]  plans_generated_3[@] plans_generated_4[@] 
print_table "Plans Visited"   plans_visited_1[@]   plans_visited_2[@]   plans_visited_3[@] plans_visited_4[@]   
print_table "Dead Ends"       dead_ends_1[@]       dead_ends_2[@]       dead_ends_3[@] dead_ends_4[@]       
print_table "Plan Length"     plan_length_1[@]     plan_length_2[@]     plan_length_3[@] plan_length_4[@]     

powershell.exe -c "(New-Object Media.SoundPlayer 'C:/Users/User/Videos/Projects/learninggodot/art/gameover.wav').PlaySync()"
